...
| 模型 ID | 说明 |
|---------|------|
| `BAAI/bge-m3` | 多语言向量模型，免费，推荐 |
| `Pro/BAAI/bge-m3` | bge-m3 的高性能版本，付费 |
| `BAAI/bge-large-zh-v1.5` | 中文向量模型，免费，纯中文场景效果好 |
| `Qwen/Qwen3-Embedding-8B` | 通义千问向量模型，效果更好，付费但价格极低 |

**Rerank 模型（重排序）：**

| 模型 ID | 说明 |
|---------|------|
| `BAAI/bge-reranker-v2-m3` | 多语言重排序模型，免费，推荐 |
| `Qwen/Qwen3-Reranker-8B` | 通义千问重排序模型，效果更好，付费但价格极低 |
...
- **默认**：开启
- **作用**：根据当前已向量化的总结数量，自动计算合适的 topK 和 topN 值
- **计算规则**：topK = 总结数量 × 25%（范围 10-100），topN = topK × 30%（范围 3-30）
- **示例**：
  - 50 条总结 → topK=13, topN=4
  - 100 条总结 → topK=25, topN=8
  - 200 条总结 → topK=50, topN=15
  - 400+ 条总结 → topK=100, topN=30（上限）
- **建议**：大多数情况下保持开启即可，系统会随着对话积累自动扩大检索范围

关闭自动调整后，可以手动设置以下两个参数：
