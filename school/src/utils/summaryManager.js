...
const SUMMARY_AI_SYSTEM_DATA = {
  ...
  rules: [
    ...
    '专注于剧情脉络、关键转折和角色关系变化',
    '如果用户指定了格式字段（如"日期|…"、"标题|…"），必须严格按指定字段格式输出，不要改用散文或自由格式'
  ]
}
...
const MINOR_SUMMARY_DATA = {
  ...
  output_example: `<minor_summary>日期|2025年4月15日 14:30
标题|图书馆偶遇与数学笔记交换
地点|学校图书馆二楼自习区
人物|我、林晓薇、张浩然
描述|我在图书馆复习数学时偶遇林晓薇，两人讨论了期中考试范围。林晓薇主动借出自己整理的笔记并约定周五归还。张浩然路过时打了个招呼，提到社团下周要开会讨论新活动方案。
人物关系|我与林晓薇关系更进一步（从普通同学到愿意互借笔记）
重要信息|期中考试范围确定为第三章到第七章；社团下周有重要会议
角色意图|林晓薇希望和我一起复习；张浩然计划准备社团会议提案
互动内容|林晓薇借出数学笔记，约定周五放学后在图书馆归还
待办事项|周五放学后在图书馆归还林晓薇的数学笔记</minor_summary>`,
  input: '{content}',
...
const MAJOR_SUMMARY_DATA = {
  ...
  requirements: [
    ...
    '字数控制在300-500字',
    '直接输出总结内容，不要添加任何前言或解释',
    '保持与小总结相同的字段格式（日期|…、标题|…、地点|…等），合并后仍按字段输出'
  ],
  input: '{summaries}',
...
export async function generateBatchSummaries(chatLog, onProgress, options = {}) {
  ...
  for (let i = 0; i < totalGroups; i++) {
    ...
    if (group.length === 1) {
      ...
      const formatFieldsText = SUMMARY_FORMAT_FIELDS.map((f, i) => `    ${f}`).join('\n')
