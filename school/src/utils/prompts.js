  ...
  try {
    ...
    if (mpStore.isMultiplayerActive) {
      ...
      if (mpStore.roomGameTime && gameState.world?.gameTime) {
        ...
        const localMinutes = ((local.year || 2024) * 525600) + ((local.month || 1) * 43800) + ((local.day || 1) * 1440) + ((local.hour || 0) * 60) + (local.minute || 0)
        const roomMinutes = ((room.year || 2024) * 525600) + ((room.month || 1) * 43800) + ((room.day || 1) * 1440) + ((room.hour || 0) * 60) + (room.minute || 0)
        const gapMinutes = roomMinutes - localMinutes
        if (gapMinutes > 30) {
          let gapStr = ''
          if (gapMinutes >= 43800) {
            const months = Math.floor(gapMinutes / 43800)
            gapStr = `${months}个月`
