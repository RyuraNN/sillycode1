...
用于更新角色的衣着描述。当剧情中角色换装时使用。

> 注：玩家的衣着在 prompt 中会优先使用装备栏的物品。`outfit` 指令设置的是"基础衣着"，
> 仅在对应装备槽位为空时生效。NPC 没有装备系统，`outfit` 是唯一的衣着来源。
...
如果角色换装后又换回原来的衣服，请再次输出对应的描述。

---

## 联机系统指令 (Multiplayer Commands)

联机模式下新增以下 XML 指令，用于多玩家环境中的 NPC 记忆共享和 NPC 转移。

### 1. NPC 记忆记录 (`npc_memory`)

当 NPC 与玩家互动后产生了值得记住的短期记忆时使用。该记忆会同步到同一房间的所有玩家。

```xml
<npc_memory npc="NPC姓名" time="4月1日 14:30">
记忆内容（以NPC第一人称描述）
</npc_memory>
```
...
```xml
<npc_memory npc="后藤一里" time="4月3日 12:15">
午休时在天台遇到了那个转学生，他似乎对吉他很感兴趣。我们聊了一会儿音乐的事。
</npc_memory>
```

> 注：每个 NPC 最多保留最近 50 条记忆。联机模式下，这些记忆会被注入到所有玩家的 AI 提示词中，确保 NPC 对不同玩家保持一致的记忆。

### 2. NPC 转移 (`transfer_npc`)

当一个 NPC 需要从当前玩家的场景转移到另一个玩家所在位置时使用。常见场景：NPC 说"我要去找某某人"。

```xml
<transfer_npc npc="NPC姓名" to="目标玩家名" reason="转移原因" />
```

**参数说明：**
- `npc`：要转移的 NPC 姓名（必需）
- `to`：目标玩家的名称（必需）
- `reason`：转移原因（可选，会显示给目标玩家）

**示例：**

```xml
<transfer_npc npc="后藤一里" to="小明" reason="约好了一起去图书馆" />
```

> 注：转移后，该 NPC 的 isAlive 状态会在原玩家场景中设为 false，在目标玩家场景中设为 true。

### 3. NPC 跟随 (`npc_follow`)

当 NPC 开始或停止跟随当前玩家时使用。

```xml
<!-- 开始跟随 -->
<npc_follow npc="NPC姓名" action="start" />

<!-- 停止跟随 -->
<npc_follow npc="NPC姓名" action="stop" />
```

**参数说明：**
- `npc`：NPC 姓名（必需）
- `action`：`start` 开始跟随，`stop` 停止跟随（必需）

> 注：跟随中的 NPC 会随玩家位置变化自动移动，不受日程系统影响。

### 4. 合并对话模式 (Conversation Merge)

联机模式下，当多个玩家在同一地点时可以加入同一对话组。此时 AI 会收到合并后的多玩家输入：
...
AI 应对所有在场玩家的行动做出响应，保持叙事的连贯性。提示词中会注入以下上下文信息：

- `[合并对话模式]` — 标识当前处于多玩家合并对话
- `[本轮行动]` — 列出各玩家的行动内容
- `[共享NPC]` — 对话组中共享的 NPC 列表
- `[系统] 以下角色正在与玩家X互动，处于占用状态：...` — NPC 占用提示
- `[系统] 玩家X也在当前位置。` — 同位置玩家提示

> 注：合并对话中，跳过行动的玩家会被描述为"看着大家"。AI 不应为跳过行动的玩家生成主动行为。
