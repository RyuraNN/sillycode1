...
export class RoomIndex extends DurableObject {
  constructor(ctx, env) {
    ...
    `)
    // 迁移：为已有表添加新列
    try { this.ctx.storage.sql.exec(`ALTER TABLE public_rooms ADD COLUMN host_features TEXT`) } catch {}
    try { this.ctx.storage.sql.exec(`ALTER TABLE public_rooms ADD COLUMN status TEXT DEFAULT 'waiting'`) } catch {}
  ...
  async updatePlayerCount(roomId, count) {
    ...
    // 如果人数为 0，设置 alarm 延迟清理（60秒后清理空房间）
    if (count === 0) {
      this.scheduleAlarm(Date.now() + 60_000)
    }
  }

  async updateRoomStatus(roomId, status) {
    this.ctx.storage.sql.exec(
      `UPDATE public_rooms SET status = ?, updated_at = ? WHERE room_id = ?`,
  ...
  async alarm() {
    ...
    // 1. 清理空房间（player_count=0 且超过 60 秒未更新）
    this.ctx.storage.sql.exec(
      `DELETE FROM public_rooms WHERE player_count = 0 AND updated_at < ?`, now - 60_000
    )
  ...
  async listPublicRooms() {
    ...
    // 兜底：清理超过 60 秒的空房间
    const emptyCutoff = Date.now() - 60_000
    this.ctx.storage.sql.exec(`DELETE FROM public_rooms WHERE player_count = 0 AND updated_at < ?`, emptyCutoff)

    return [...this.ctx.storage.sql.exec(
      `SELECT room_id, room_name, host_name, game_mode, player_count, max_players, game_time, week_number, host_features, status, created_at, updated_at
